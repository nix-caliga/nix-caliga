# copied from system-manager's nix/modules/upstream/nixpkgs/openssh.nix
# changes:
#   service name: sshd-nix-caliga instead of ssh-system-manager
#   wantedBy multi-user.target instead of system-manager.target
#   masks Fedora unit names instead of Debian (sshd.* instead of ssh.*)
#   programPath option for configuring the local image ssh service
{
  lib,
  pkgs,
  config,
...

{
  ...
  options = {
    ...
    services.openssh = {
      ...
      # nix-caliga: path to the host sshd binary, used at runtime instead of the Nix store package
      programPath = lib.mkOption {
        type = lib.types.str;
        default = "/usr/sbin/sshd";
        description = "Path to the sshd binary on the base image. Used for ExecStart at runtime.";
      };
  ...
  config = lib.mkMerge [
    (lib.mkIf cfg.enable {
      ...
      # nix-caliga: mask Fedora's default sshd units (system-manager masks Debian ssh.* names)
      systemd.maskedUnits = [
        "sshd.service"
        "sshd.socket"
      ];
      ...
      # nix-caliga: service name and target changed from system-manager
      # Service definition taken from ubuntu
      systemd.services."sshd-nix-caliga" = {
        ...
        unitConfig = {
          ...
          After = "network.target auditd.service sshd-keygen.target";
          Wants = "sshd-keygen.target";
          ConditionPathExists = "!/etc/ssh/sshd_not_to_be_run";
        };
        serviceConfig = {
          EnvironmentFile = "-/etc/default/ssh";
          ExecStartPre = "${cfg.programPath} -t";
          ExecStart = "${cfg.programPath} -D $SSHD_OPTS";
