...
When `caliga.core.selinux.enable` is true, nix-caliga creates a file at `/etc/selinux/targeted/contexts/files/file_contexts.local` with rules that label common nix store subdirectories.
This file is placed using `environment.etc` as a real file, as SELinux needs it before the nix store is available.

## Nix store labels
The default nix store context rules (enabled by `selinux.nixStoreContexts.enable`) cover the standard layout used by nix-caliga:
...
The `selinux.labelServiceExecs` option accepts a list of systemd service names and it will extract all `Exec*` paths (ExecStart, ExecStartPre, etc.) from those services, and add `bin_t` rules for all nix store paths.

## SELinux enforcement
By default, the base image's SELinux enforcement mode is left unchanged. The `selinux.enforcementMode` option can override it by writing `/etc/selinux/config` with the specified mode.

## Custom file contexts
Additional file context rules can be added with `selinux.fileContexts`:
```nix
